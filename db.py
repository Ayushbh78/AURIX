...
def _sb_load() -> dict:
    ...
    d["goals"] = [{
        ...
        "roadmap_stages": r.get("roadmap_stages") or [],
        "created":      _d(r.get("created","")),
    } for r in rows]
...
def _goal_row(g):
    return {
        ...
        "roadmap_stages":  g.get("roadmap_stages") or [],
...
def _w_notification_history(sb, data, prev):
    """Append-only notification history sync."""
    items = data.get("notification_history", [])
    prev_ids = {n.get("id") for n in prev.get("notification_history", [])}
    for n in items:
        nid = n.get("id")
        if nid and nid not in prev_ids:
            try:
                sb.table("notification_history").upsert({
                    "id": nid, "title": n.get("title", ""),
                    "body": n.get("body", ""), "type": n.get("type", "info"),
                    "url": n.get("url", "/"), "ts": n.get("ts", ""),
                    "read": n.get("read", False)
                }).execute()
            except:
                pass
    # Handle mark-read updates
    prev_map = {n.get("id"): n for n in prev.get("notification_history", [])}
    for n in items:
        nid = n.get("id")
        if nid in prev_map and prev_map[nid].get("read") != n.get("read"):
            try:
                sb.table("notification_history").update({"read": n.get("read", False)}).eq("id", nid).execute()
            except:
                pass
    # Handle deletes (clear all)
    if not items and prev.get("notification_history"):
        try:
            sb.table("notification_history").delete().neq("id", 0).execute()
        except:
            pass
