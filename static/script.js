...
function openRoadmap(gid, goalTitle) {
  ...
  // Push a history state so browser Back / PWA back-swipe closes the panel
...
function _roadmapHtmlTemplate() {
  ...
  <div class="roadmap-panel" id="roadmap-panel">
    ...
    <!-- Progress bar with tick indicators -->
    ...
    <!-- Exam Link section inside roadmap -->
...
function closeRoadmapPanel() {
  ...
  if (panel) {
    panel.style.animation = 'rmPanelOut 0.28s cubic-bezier(0.4,0,1,1) both';
  }
  ...
  // Clean up hash if it belongs to this roadmap
...
// Handle browser Back button / PWA back-swipe closing the roadmap
window.addEventListener('popstate', function(e) {
  const ov = document.getElementById('roadmap-overlay');
  if (ov) {
    // Back was pressed while panel open — animate it closed (no new history entry)
    const panel = document.getElementById('roadmap-panel');
    ...
    // Forward navigation restored a roadmap state
...
// ── Load & render exam link bar inside roadmap ─────
function _loadRoadmapExams(gid) {
  fetch(`/roadmap/${gid}/exam-info`)
    ...
    .then(d => {
      ...
      let html = '<span class="rm-exam-link-label">🔗 Exam</span>';

      if (d.linked && d.linked.length > 0) {
        d.linked.forEach(ex => {
          html += `<span class="rm-exam-pill" id="rm-ex-pill-${ex.id}">
      ...
      if (d.available && d.available.length > 0) {
        html += `<div class="rm-exam-link-select-wrap">
          ...
          <button class="rm-exam-link-btn" onclick="rmLinkExam(${gid})">Link</button>
        </div>`;
      }
  ...
  fetch(`/link-exam-to-goal/${gid}`, {method:'POST', body:fd})
    ...
    .then(d => {
      if (d.status === 'ok') {
        ...
        _loadRoadmapExams(gid);
        // Refresh goal cards in background
...
function _updateProgressBar(pct, done, total) {
  ...
  if (ticks && total > 0) {
    ...
    for (let i = 0; i < total; i++) {
      ...
      tick.innerHTML = i < done ? '<svg viewBox="0 0 12 12" fill="none"><polyline points="2,6 5,9 10,3" stroke="#fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>' : '';
...
function _buildSnakeRoad(container, stages, activeIdx) {
  // Group stages into rows of 3: even rows go L→R, odd rows go R→L (snake pattern)
  const ROW_SIZE = 3;
  const rows = [];
  ...
  rows.forEach((rowStages, rowIdx) => {
    ...
    // For odd rows, reverse display order (snake direction)
    const displayStages = isEven ? rowStages : [...rowStages].reverse();

    // Row wrapper
    const row = document.createElement('div');
    row.className = 'rm-road-row';

    displayStages.forEach((s, colIdx) => {
      // Map display index back to absolute stage index
      ...
      const nextStage = stages[nextAbsIdx];
      const connDone = isDone && nextStage && nextStage.completed;

      // Node wrapper
      const node = document.createElement('div');
      node.className = 'rm-road-node';
      node.id = 'rm-stage-' + s.id;

      // Circle state class
      let circleClass = 'rm-badge-circle';
      if (isDone)   circleClass += ' rm-badge-done';
      if (isActive) circleClass += ' rm-badge-active';
      ...
      const completedAt = s.completed_at
        ...
        : '';
      let statusTag = '';
      if (isDone)        statusTag = `<div class="rm-badge-status rm-badge-status-done">✓ Done${completedAt ? ' · ' + completedAt : ''}</div>`;
      else if (isActive) statusTag = `<div class="rm-badge-status rm-badge-status-active">▶ Active</div>`;
      else               statusTag = `<div class="rm-badge-status rm-badge-status-pending">Stage ${absIdx + 1}</div>`;

      node.innerHTML = `
        <div class="rm-node-inner">
          <button class="rm-badge-card ${isDone ? 'rm-badge-card-done' : ''} ${isActive ? 'rm-badge-card-active' : ''}"
                  id="rm-badge-${s.id}" title="${_escHtml(s.title)}">
            <div class="${circleClass}" onclick="toggleRmStage(${s.id})">
              <span class="rm-circle-num">${absIdx + 1}</span>
              <span class="rm-circle-tick">
                <svg viewBox="0 0 20 20" fill="none"><polyline points="4,10 8,14 16,6" stroke="#fff" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/></svg>
              </span>
              <span class="rm-circle-dot"></span>
            </div>
            <div class="rm-badge-body">
              ${statusTag}
              <div class="rm-badge-title ${isDone ? 'rm-badge-title-done' : ''}">${_escHtml(s.title)}</div>
              ${s.description ? `<div class="rm-badge-desc">${_escHtml(s.description)}</div>` : ''}
            </div>
            <div class="rm-badge-actions">
              <button class="stage-action-btn" onclick="openRmEdit(${s.id},'${_escAttr(s.title)}','${_escAttr(s.description||'')}')" title="Edit">✏</button>
              <button class="stage-action-btn danger" onclick="deleteRmStage(${s.id})" title="Delete">✕</button>
            </div>
          </button>
        </div>
      `;
      ...
      // Horizontal connector between nodes within same row
      if (!isLastInRow) {
        const hConn = document.createElement('div');
        hConn.className = 'rm-connector-h' + (connDone ? ' rm-connector-done' : '');
        hConn.setAttribute('aria-hidden', 'true');
        hConn.innerHTML = `
          <svg viewBox="0 0 56 12" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
            <line x1="2" y1="6" x2="54" y2="6"
              stroke="${connDone ? 'var(--emerald)' : 'var(--violet)'}"
              stroke-width="2.5" stroke-dasharray="6 5" stroke-linecap="round"
              class="rm-h-line ${connDone ? 'rm-line-done' : 'rm-line-pending'}"/>
          </svg>`;
    ...
    if (rowIdx < rows.length - 1) {
      const nextRowFirstAbsIdx = (rowIdx + 1) * ROW_SIZE;
      const nextDone = stages[nextRowFirstAbsIdx] && stages[nextRowFirstAbsIdx].completed;
      const connColor = nextDone ? 'var(--emerald)' : 'var(--violet)';

      const vWrap = document.createElement('div');
      vWrap.className = 'rm-connector-v-wrap';
      vWrap.setAttribute('aria-hidden', 'true');

      // isEven row just finished L→R, so the vertical arc goes to the RIGHT side
      // isOdd row finished R→L, arc goes to the LEFT side
      const arcClass = isEven ? 'rm-conn-right' : 'rm-conn-left';

      // SVG path: smooth S-curve that dips down then arrives at the start of next row
      // Even: last node is right edge → curve goes right-side down
      // Odd:  last node is left edge  → curve goes left-side down
      const pathD = isEven
        ? 'M 8 4 C 8 4 92 4 92 30 C 92 56 8 56 8 56'
        : 'M 92 4 C 92 4 8 4 8 30 C 8 56 92 56 92 56';

      vWrap.innerHTML = `
        <div class="rm-connector-v ${arcClass}">
          <svg class="rm-curve-svg" viewBox="0 0 100 60" fill="none"
               xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
            <path d="${pathD}"
              stroke="${connColor}" stroke-width="2.5" stroke-dasharray="7 5"
              fill="none" stroke-linecap="round" stroke-linejoin="round"
              class="rm-v-path ${nextDone ? 'rm-line-done' : 'rm-line-pending'}"/>
          </svg>
        </div>`;
  ...
  container.querySelectorAll('.rm-badge-card').forEach((card, i) => {
...
function toggleRmStage(sid) {
  if (!_rmGoalId) return;
  const circle = document.querySelector(`#rm-stage-${sid} .rm-badge-circle`);
...
function _updateStatsOnly(d) {
  _el('rm-stat-done',      d.done  || 0);
  _el('rm-stat-remaining', (d.total - d.done) || 0);
  _el('rm-stat-pct',       (d.pct  || 0) + '%');
  _el('rm-pill-done',      d.done || 0);
  _updateProgressBar(d.pct || 0, d.done || 0, d.total || 0);
}
