...
function _buildSnakeRoad(container, stages, activeIdx) {
  // Group stages into rows: alternate left-to-right, right-to-left
  const ROW_SIZE = 3;
  const rows = [];
  ...
  rows.forEach((rowStages, rowIdx) => {
    ...
    displayStages.forEach((s, colIdx) => {
      ...
      const isLast   = (rowIdx * ROW_SIZE + colIdx) === stages.length - 1;
      ...
      let connectorHtml = '';
      if (!isLastInRow) {
        connectorHtml = `<div class="rm-connector-h ${isDone ? 'rm-connector-done' : ''}">
          <svg viewBox="0 0 60 10" fill="none" xmlns="http://www.w3.org/2000/svg">
            <line x1="0" y1="5" x2="60" y2="5" stroke="${isDone ? 'var(--emerald)' : 'var(--border)'}" stroke-width="2" stroke-dasharray="6 4" stroke-linecap="round"/>
          </svg>
        </div>`;
      }
      ...
      const completedAt = s.completed_at ? new Date(s.completed_at).toLocaleDateString('en-IN', {day:'numeric',month:'short'}) : '';
    ...
    // Vertical connector between rows (curve connector)
    if (rowIdx < rows.length - 1) {
      const vConn = document.createElement('div');
      vConn.className = 'rm-connector-v-wrap';
      const nextDone = stages[(rowIdx + 1) * ROW_SIZE]?.completed;
      vConn.innerHTML = `
        <div class="rm-connector-v ${isEven ? 'rm-conn-right' : 'rm-conn-left'}">
          <svg class="rm-curve-svg" viewBox="0 0 120 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            ${isEven
              ? `<path d="M 10 5 Q 110 5 110 30 Q 110 55 10 55" stroke="${nextDone ? 'var(--emerald)' : 'var(--violet)'}" stroke-width="2" stroke-dasharray="7 5" fill="none" stroke-linecap="round"/>`
              : `<path d="M 110 5 Q 10 5 10 30 Q 10 55 110 55" stroke="${nextDone ? 'var(--emerald)' : 'var(--violet)'}" stroke-width="2" stroke-dasharray="7 5" fill="none" stroke-linecap="round"/>`
            }
          </svg>
        </div>
      `;
      container.appendChild(vConn);
    }
  });
}
