...
function openRoadmap(gid, goalTitle) {
  ...
  if (!document.getElementById('roadmap-overlay')) {
    fetch('/static/roadmap_fragment.html')
      .catch(() => null)
      .then(() => {
        _injectRoadmapHtml(_rmGoalId, goalTitle);
      });
  } else {
...
function _roadmapHtmlTemplate() {
  ...
  <div class="roadmap-panel" id="roadmap-panel">
    ...
    <div class="roadmap-progress-arc-wrap">
      <svg class="roadmap-arc-svg" viewBox="0 0 220 130" fill="none" xmlns="http://www.w3.org/2000/svg">
        <defs>
          <linearGradient id="rmGradient" x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%"   stop-color="#7c3aed"/>
            <stop offset="100%" stop-color="#4338ca"/>
          </linearGradient>
        </defs>
        <path class="roadmap-arc-track" d="M 20 120 A 90 90 0 0 1 200 120" stroke="var(--bg4)" stroke-width="12" stroke-linecap="round" fill="none"/>
        <path class="roadmap-arc-fill" id="rm-arc-fill" d="M 20 120 A 90 90 0 0 1 200 120"
              stroke="url(#rmGradient)" stroke-width="12" stroke-linecap="round" fill="none"
              stroke-dasharray="283" stroke-dashoffset="283"/>
      </svg>
      <div class="roadmap-arc-label" id="rm-arc-pct">0%</div>
      <div class="roadmap-arc-sub"  id="rm-arc-sub">0 of 0 stages</div>
    </div>
    ...
    <div class="roadmap-stages-wrap" id="rm-stages-wrap">
      ...
      <div class="roadmap-stages-list" id="rm-stages-list"></div>
    </div>
  </div>
...
function _renderRoadmap(d) {
  ...
  // Arc
  _updateArc(d.pct || 0, d.done || 0, d.total || 0);

  // Empty state
  const empty = document.getElementById('rm-empty-state');
  const list  = document.getElementById('rm-stages-list');
  ...
  if (list) {
    ...
    d.stages.forEach((s, idx) => {
      list.appendChild(_buildStageCard(s, idx, d.active_idx));
    });
  }
}
...
function _updateArc(pct, done, total) {
  const arc = document.getElementById('rm-arc-fill');
  const lbl = document.getElementById('rm-arc-pct');
  const sub = document.getElementById('rm-arc-sub');
  const MAX_DASH = 283;
  if (arc) {
    const offset = MAX_DASH - (MAX_DASH * Math.min(pct, 100) / 100);
    arc.style.strokeDashoffset = offset;
  }
  if (lbl) lbl.textContent = pct + '%';
  if (sub) sub.textContent = done + ' of ' + total + ' stage' + (total !== 1 ? 's' : '');
}

function _buildStageCard(s, idx, activeIdx) {
  const isActive = idx === activeIdx && !s.completed;
  const isDone   = s.completed;

  const wrap = document.createElement('div');
  wrap.className = 'roadmap-stage-card';
  wrap.id = 'rm-stage-' + s.id;

  // Circle
  let circleClass = 'roadmap-stage-circle';
  if (isDone)   circleClass += ' done-stage';
  if (isActive) circleClass += ' active-stage';

  const completedAt = s.completed_at ? new Date(s.completed_at).toLocaleDateString('en-IN', {day:'numeric',month:'short',year:'numeric'}) : '';

  let orderLblClass = 'stage-order-lbl';
  let bodyClass     = 'roadmap-stage-body';
  let orderLblText  = 'Stage ' + (idx + 1);
  if (isDone)   { orderLblClass += ' done-lbl';   bodyClass += ' stage-done-body';   orderLblText = 'Completed ✓'; }
  if (isActive) { orderLblClass += ' active-lbl'; bodyClass += ' stage-active-body'; orderLblText = '▶ Active Stage'; }

  wrap.innerHTML = `
    <div class="${circleClass}" onclick="toggleRmStage(${s.id})" title="Click to toggle completion">
      <span class="stage-num">${idx + 1}</span>
      <span class="stage-tick">✓</span>
      <span class="stage-active-dot"></span>
    </div>
    <div class="${bodyClass}" id="rm-body-${s.id}">
      <div class="stage-body-top">
        <div class="stage-title-area">
          <div class="${orderLblClass}" id="rm-orderlbl-${s.id}">${orderLblText}</div>
          <div class="stage-title-text${isDone ? ' done-text' : ''}" id="rm-title-${s.id}">${_escHtml(s.title)}</div>
          ${s.description ? `<div class="stage-desc-text" id="rm-desc-${s.id}">${_escHtml(s.description)}</div>` : `<div class="stage-desc-text" id="rm-desc-${s.id}" style="display:none"></div>`}
          ${isDone && completedAt ? `<div class="stage-completed-at" id="rm-cat-${s.id}"><span>✓</span><span>${completedAt}</span></div>` : `<div class="stage-completed-at" id="rm-cat-${s.id}" style="display:none"></div>`}
        </div>
...
function toggleRmStage(sid) {
  if (!_rmGoalId) return;
  const circle = document.querySelector(`#rm-stage-${sid} .roadmap-stage-circle`);
  ...
  fetch(`/roadmap/${_rmGoalId}/toggle-stage/${sid}`, {method: 'POST'})
    ...
    .then(d => {
      if (d.status === 'ok') {
        // Refresh full roadmap to get proper active_idx
    ...
    .then(d => {
      if (d.status === 'ok') {
        ...
        if (card) {
          ...
          const s = document.createElement('style');
          s.textContent = '@keyframes rmStageOut{from{opacity:1;transform:translateX(0)}to{opacity:0;transform:translateX(30px)}}';
          document.head.appendChild(s);
