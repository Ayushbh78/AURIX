...
function toggleAIPanel() {
  ...
  if (fab) fab.classList.toggle('panel-open', aiPanelOpen);
  if (aiPanelOpen) {
...
// AI state
let aiPending   = null;   // pending confirmation data from bot
let awaitingYN  = false;  // waiting for yes/no

function sendAIMessage() {
  const inp = document.getElementById('ai-input');
  const raw = inp ? inp.value.trim() : '';
  if (!raw) return;
  appendAIMessage(raw, 'user');
  inp.value = '';

  // Handle yes/no confirmation
  const lc = raw.toLowerCase();
  if (awaitingYN && aiPending) {
    if (['yes','y','confirm','ok','haan','ha','✓','correct','proceed'].some(k => lc === k || lc.startsWith(k))) {
      awaitingYN = false;
      const typing = appendAIMessage('●●●', 'bot', true);
      fetch('/api/ai-chat', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({message: raw, confirm: true, pending: aiPending})
      }).then(r=>r.json()).then(d=>{
        typing.remove();
        aiPending = null;
        appendAIMessage(d.reply, 'bot');
        if (speechSynth) speakReply(d.reply.substring(0,120));
        if (d.action) handleAIAction(d.action);
      }).catch(()=>{ typing.remove(); appendAIMessage('Something went wrong.','bot'); });
      return;
    } else if (['no','n','cancel','nahi','nope','nah'].some(k => lc === k || lc.startsWith(k))) {
      awaitingYN = false; aiPending = null;
      fetch('/api/ai-chat', {
        method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({message: raw, confirm: false, pending: null})
      }).then(r=>r.json()).then(d=>{ appendAIMessage(d.reply,'bot'); });
      return;
    }
  }
  ...
  fetch('/api/ai-chat', {
    ...
    body: JSON.stringify({message: raw, lang: 'auto'})
  })
  .then(r => r.json())
  .then(d => {
    typing.remove();
    // Render bold markdown in reply
    const rendered = (d.reply||'').replace(/\*\*(.+?)\*\*/g,'<strong>$1</strong>').replace(/\n/g,'<br>');
    appendAIMessage(rendered, 'bot', false, true);
    if (speechSynth) speakReply(d.reply.substring(0,120));
    if (d.needs_confirm && d.pending) {
      aiPending  = d.pending;
      awaitingYN = true;
    }
    if (d.action) handleAIAction(d.action);
  })
  .catch(() => { typing.remove(); appendAIMessage('Sorry, something went wrong.', 'bot'); });
}

function appendAIMessage(text, role, isTyping=false, isHTML=false) {
...
function handleAIAction(action) {
  ...
  if (action.type === 'start_focus') {
    ...
    showToast('Starting ' + mins + 'm focus timer — go to Focus Timer page', 'info');
  } else if (action.type === 'expense_added') {
    showToast('Expense logged: ₹' + action.amount, 'success');
  } else if (action.type === 'habit_created') {
    showToast('Habit added: ' + action.name, 'success');
  } else if (action.type === 'exam_created') {
    showToast('Exam created: ' + action.name, 'success');
  } else if (action.type === 'goal_created') {
    showToast('Goal created: ' + action.name, 'success');
  } else if (action.type === 'exercise_logged') {
    showToast('Workout logged: ' + action.workout, 'success');
  }
}
...
function speakReply(text) {
  ...
  const plain = text.replace(/<[^>]+>/g, '').substring(0, 120);
...
function toggleVoice() {
  ...
  if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
    showToast('Voice not supported in this browser', 'warn');
    return;
  }
  if (isListening) {
  ...
  recognition.lang = 'en-IN'; // supports Hindi too via en-IN
  recognition.onstart = () => {
    isListening = true;
  ...
  recognition.onend = () => {
    ...
    const inp = document.getElementById('ai-input');
    if (inp && inp.value.trim()) setTimeout(sendAIMessage, 400);
  };
...
document.addEventListener('click', (e) => {
  const panel = document.getElementById('ai-panel');
  const fab   = document.getElementById('ai-fab');
  if (aiPanelOpen && panel && fab && !panel.contains(e.target) && !fab.contains(e.target)) {
    toggleAIPanel();
  }
});
