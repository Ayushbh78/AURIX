...
-- Run this if the table already exists to add missing columns:
...
-- ══════════════════════════════════════════════════════════════════
-- MIGRATION: Add roadmap_stages to goals table (safe, backward-compatible)
-- Run these only if upgrading an existing schema; the CREATE TABLE
-- above already includes roadmap_stages for fresh installs.
-- ══════════════════════════════════════════════════════════════════

alter table goals
  add column if not exists roadmap_stages jsonb default '[]';

-- Index for faster lookups when goals table grows large
create index if not exists idx_goals_linked_exams
  on goals using gin (linked_exams);

create index if not exists idx_goals_roadmap_stages
  on goals using gin (roadmap_stages);

-- Ensure existing rows have default value (safe no-op if already set)
update goals
  set roadmap_stages = '[]'
  where roadmap_stages is null;

update goals
  set linked_exams = '[]'
  where linked_exams is null;

-- ══════════════════════════════════════════════════════════════════
-- MIGRATION: Sub-Roadmap (Substages) support
-- Each roadmap stage can now have an array of substages stored as
-- nested JSONB inside the roadmap_stages column. No new table needed
-- since substages live inside the stage object itself.
--
-- For FRESH installs: The roadmap_stages column already accepts any
-- JSONB structure including substages arrays.
--
-- For EXISTING installs: Run the block below to back-fill a
-- 'substages' key into every stage object that lacks one.
-- ══════════════════════════════════════════════════════════════════

-- Back-fill substages: [] into every roadmap stage that doesn't
-- already have one. This is a safe, idempotent operation.
update goals
set roadmap_stages = (
  select jsonb_agg(
    case
      when (stage ? 'substages') then stage
      else stage || '{"substages": []}'::jsonb
    end
  )
  from jsonb_array_elements(roadmap_stages) as stage
)
where jsonb_array_length(roadmap_stages) > 0;
