...
def habits():
    ...
    for h in habits:
        ...
        if sub_tasks:
            ...
            h['subtask_total'] = 0
            h['incomplete_tasks'] = []
        # Reminder metadata (stored server-side as backup)
        h['has_reminder']   = bool(h.get('reminder_times') and h.get('reminder_enabled', True))
        h['reminder_times'] = ', '.join(h.get('reminder_times', []))
...
def notifications():
    data   = load_data()
    habits = data.get('habits', [])
    return render_template('notifications.html', habits=habits)

@app.route('/api/insights-data')
def api_insights_data():
    """Return habit analytics for weekly/monthly insight notifications."""
    ...
    for i in range(7):
        ...
        week_total += total_habits
        week_done  += len(habit_logs.get(d, []))

    habit_completion_pct = round((week_done / week_total * 100) if week_total else 0)

    # Completed/missed today
    done_today   = set(habit_logs.get(today, []))
    completed_ids = list(done_today)
    missed_ids    = [h['id'] for h in habits if h['id'] not in done_today]

    # Best streak across all habits
    best_streak = 0
    for h in habits:
        s = calc_streak(data, h['id'])
        if s > best_streak:
            best_streak = s

    # Most consistent habit (last 30 days)
    most_consistent = None
    best_rate = 0
    for h in habits:
        count = sum(1 for i in range(30)
                    if h['id'] in habit_logs.get((today_d - timedelta(days=i)).isoformat(), []))
        rate = count / 30 * 100
        if rate > best_rate:
            best_rate = rate
            most_consistent = h['name']

    return jsonify({
        'total_habits':          total_habits,
        'completed_habits':      len(done_today),
        'missed_habits':         len(missed_ids),
...
@app.route('/api/habit-reminder', methods=['POST'])
def api_save_habit_reminder():
    """Save habit reminder config server-side (backup — client also stores in localStorage)."""
    data = load_data()
    body = request.get_json(silent=True) or {}
    hid  = body.get('habit_id')
    if not hid:
        return jsonify({'status': 'error', 'msg': 'Missing habit_id'}), 400

    # Store reminder config on the habit object
    for h in data.get('habits', []):
        if h['id'] == int(hid):
            h['reminder_times']   = body.get('times', [])
            h['reminder_enabled'] = body.get('enabled', True)
            h['reminder_sound']   = body.get('sound', 'default')
            h['reminder_message'] = body.get('message', '')
            break
    save_data(data)
    return jsonify({'status': 'ok'})
