...
# ── Serve SW and manifest at root scope for PWA ───────────────
@app.route('/sw.js')
def service_worker():
...
@app.route('/roadmap-page/<int:gid>')
def roadmap_page(gid):
    """Full-screen dedicated roadmap page for a goal."""
    data = load_data()
    goal = next((g for g in data['goals'] if g['id'] == gid), None)
    if not goal:
        return redirect(url_for('goals'))
    return render_template('roadmap_page.html', gid=gid, goal=goal)

@app.route('/roadmap/<int:gid>')
def get_roadmap(gid):
    """Return roadmap stages for a goal as JSON."""
    data = load_data()
    for g in data['goals']:
        if g['id'] == gid:
            stages = g.get('roadmap_stages', [])
            # Ensure every stage has a substages list
            for s in stages:
                s.setdefault('substages', [])
            total = len(stages)
            done  = sum(1 for s in stages if s.get('completed', False))
            pct   = round(done / total * 100) if total else 0
            active_idx = next((i for i, s in enumerate(stages) if not s.get('completed', False)), total)
            return jsonify({
                'status': 'ok',
    ...
    return jsonify({'status': 'error', 'msg': 'Goal not found'}), 404

@app.route('/roadmap/<int:gid>/add-stage', methods=['POST'])
def add_roadmap_stage(gid):
    data = load_data()
    title = (request.json or {}).get('title', '').strip() if request.is_json else request.form.get('title', '').strip()
    if not title:
        return jsonify({'status': 'error', 'msg': 'Title required'}), 400
    for g in data['goals']:
        if g['id'] == gid:
            stages = g.setdefault('roadmap_stages', [])
            new_id = max((s['id'] for s in stages), default=0) + 1
            stage = {
                'id': new_id,
                'title': title,
                'description': (request.json or {}).get('description', '') if request.is_json else request.form.get('description', ''),
                'completed': False,
                'completed_at': None,
                'created_at': datetime.now(_IST).isoformat(),
                'order': len(stages) + 1,
...
@app.route('/roadmap/<int:gid>/edit-stage/<int:sid>', methods=['POST'])
def edit_roadmap_stage(gid, sid):
    data = load_data()
    payload = request.json or {}
    title = payload.get('title', '').strip() if request.is_json else request.form.get('title', '').strip()
    desc  = payload.get('description', '') if request.is_json else request.form.get('description', '')
    if not title:
        return jsonify({'status': 'error', 'msg': 'Title required'}), 400
    for g in data['goals']:
        if g['id'] == gid:
            for s in g.get('roadmap_stages', []):
                if s['id'] == sid:
                    s['title'] = title
                    s['description'] = desc
                    save_data(data)
                    return jsonify({'status': 'ok', 'stage': s})
    return jsonify({'status': 'error'}), 404

@app.route('/roadmap/<int:gid>/delete-stage/<int:sid>', methods=['POST'])
def delete_roadmap_stage(gid, sid):
    data = load_data()
    for g in data['goals']:
        if g['id'] == gid:
            before = len(g.get('roadmap_stages', []))
            g['roadmap_stages'] = [s for s in g.get('roadmap_stages', []) if s['id'] != sid]
            if len(g.get('roadmap_stages', [])) < before:
                save_data(data)
                return jsonify({'status': 'ok'})
    return jsonify({'status': 'error'}), 404

@app.route('/roadmap/<int:gid>/toggle-stage/<int:sid>', methods=['POST'])
def toggle_roadmap_stage(gid, sid):
    data = load_data()
    for g in data['goals']:
        if g['id'] == gid:
            for s in g.get('roadmap_stages', []):
                if s['id'] == sid:
                    ...
                    s['completed_at'] = datetime.now(_IST).isoformat() if s['completed'] else None
                    save_data(data)
                    stages = g.get('roadmap_stages', [])
                    total = len(stages)
                    done  = sum(1 for st in stages if st.get('completed', False))
                    pct   = round(done / total * 100) if total else 0
                    return jsonify({'status': 'ok', 'completed': s['completed'], 'pct': pct, 'done': done, 'total': total})
    return jsonify({'status': 'error'}), 404

@app.route('/roadmap/<int:gid>/reorder', methods=['POST'])
def reorder_roadmap_stages(gid):
    data  = load_data()
    order = (request.json or {}).get('order', [])  # list of stage ids
    for g in data['goals']:
        if g['id'] == gid:
            stages = g.get('roadmap_stages', [])
            id_map = {s['id']: s for s in stages}
            g['roadmap_stages'] = [id_map[i] for i in order if i in id_map]
            for idx, s in enumerate(g['roadmap_stages']):
                s['order'] = idx + 1
            save_data(data)
            return jsonify({'status': 'ok'})
    return jsonify({'status': 'error'}), 404

# ─── Sub-Roadmap (Substages) ──────────────────────────────────────────────────

@app.route('/roadmap/<int:gid>/stage/<int:sid>/substages')
def get_substages(gid, sid):
    """Return substages for a specific roadmap stage."""
    data = load_data()
    for g in data['goals']:
        if g['id'] == gid:
            for s in g.get('roadmap_stages', []):
                if s['id'] == sid:
                    substages = s.get('substages', [])
                    total = len(substages)
                    done  = sum(1 for ss in substages if ss.get('completed', False))
                    pct   = round(done / total * 100) if total else 0
                    return jsonify({
                        'status': 'ok',
                        'substages': substages,
    ...
    return jsonify({'status': 'error', 'msg': 'Stage not found'}), 404

@app.route('/roadmap/<int:gid>/stage/<int:sid>/add-substage', methods=['POST'])
def add_substage(gid, sid):
    """Add a substage to a roadmap stage."""
    data = load_data()
    payload = request.json or {}
    ...
    for g in data['goals']:
        if g['id'] == gid:
            for s in g.get('roadmap_stages', []):
                if s['id'] == sid:
                    substages = s.setdefault('substages', [])
                    new_id = max((ss['id'] for ss in substages), default=0) + 1
                    substage = {
                        'id': new_id,
                        'title': title,
                        'description': desc,
                        ...
                        'completed_at': None,
                        'created_at': datetime.now(_IST).isoformat(),
                        'order': len(substages) + 1,
                    }
                    substages.append(substage)
                    save_data(data)
                    return jsonify({'status': 'ok', 'substage': substage})
    return jsonify({'status': 'error', 'msg': 'Stage not found'}), 404

@app.route('/roadmap/<int:gid>/stage/<int:sid>/edit-substage/<int:ssid>', methods=['POST'])
def edit_substage(gid, sid, ssid):
    """Edit a substage title/description."""
    data = load_data()
    payload = request.json or {}
    title = payload.get('title', '').strip()
    desc  = payload.get('description', '')
                ...
                if s['id'] == sid:
                    for ss in s.get('substages', []):
                        if ss['id'] == ssid:
                            ...
                            ss['description'] = desc
                            save_data(data)
                            return jsonify({'status': 'ok', 'substage': ss})
    return jsonify({'status': 'error'}), 404

@app.route('/roadmap/<int:gid>/stage/<int:sid>/delete-substage/<int:ssid>', methods=['POST'])
def delete_substage(gid, sid, ssid):
    """Delete a substage from a roadmap stage."""
    data = load_data()
    for g in data['goals']:
        if g['id'] == gid:
            for s in g.get('roadmap_stages', []):
                if s['id'] == sid:
                    before = len(s.get('substages', []))
                    s['substages'] = [ss for ss in s.get('substages', []) if ss['id'] != ssid]
                    if len(s.get('substages', [])) < before:
                        save_data(data)
                        return jsonify({'status': 'ok'})
    return jsonify({'status': 'error'}), 404

@app.route('/roadmap/<int:gid>/stage/<int:sid>/toggle-substage/<int:ssid>', methods=['POST'])
def toggle_substage(gid, sid, ssid):
    """Toggle completion of a substage."""
    data = load_data()
    for g in data['goals']:
        if g['id'] == gid:
            for s in g.get('roadmap_stages', []):
                if s['id'] == sid:
                    for ss in s.get('substages', []):
                        if ss['id'] == ssid:
                            ss['completed'] = not ss.get('completed', False)
                            ss['completed_at'] = datetime.now(_IST).isoformat() if ss['completed'] else None
                            save_data(data)
                            substages = s.get('substages', [])
                            total = len(substages)
                            done  = sum(1 for x in substages if x.get('completed', False))
                            pct   = round(done / total * 100) if total else 0
                            return jsonify({
                                'status': 'ok',
...
@app.route('/roadmap/<int:gid>/exam-info')
def roadmap_exam_info(gid):
    """Return linked and available exams for the roadmap exam-link bar."""
    data = load_data()
    goal = next((g for g in data['goals'] if g['id'] == gid), None)
    if not goal:
        return jsonify({'status': 'error', 'msg': 'Goal not found'}), 404
    linked_ids = goal.get('linked_exams', [])
    all_exams  = data.get('exams', [])
    linked     = [{'id': e['id'], 'name': e.get('name', '')} for e in all_exams if e['id'] in linked_ids]
    available  = [{'id': e['id'], 'name': e.get('name', '')} for e in all_exams if e['id'] not in linked_ids]
    return jsonify({'status': 'ok', 'linked': linked, 'available': available})
...
@app.route('/api/habit/<int:hid>/reminders', methods=['GET'])
def get_habit_reminders(hid):
    data = load_data()
    habit = next((h for h in data['habits'] if h['id'] == hid), None)
    if not habit:
        return jsonify({'error': 'Not found'}), 404
    reminders = habit.get('reminders', [])
    return jsonify({'reminders': reminders})

@app.route('/api/habit/<int:hid>/reminders', methods=['POST'])
def add_habit_reminder(hid):
    data = load_data()
    habit = next((h for h in data['habits'] if h['id'] == hid), None)
    if not habit:
        return jsonify({'error': 'Not found'}), 404
    if 'reminders' not in habit:
        habit['reminders'] = []
    rid = max((r.get('id', 0) for r in habit['reminders']), default=0) + 1
    body = request.get_json(force=True) or {}
    time_val = body.get('time', '09:00')  # HH:MM 24h
    label = body.get('label', '')
    schedule = body.get('schedule', 'everyday')  # 'everyday' or ISO date
    enabled = body.get('enabled', True)
    reminder = {
        'id': rid, 'time': time_val, 'label': label,
...
@app.route('/api/habit/<int:hid>/reminders/<int:rid>', methods=['PUT'])
def edit_habit_reminder(hid, rid):
    data = load_data()
    habit = next((h for h in data['habits'] if h['id'] == hid), None)
    if not habit:
        return jsonify({'error': 'Not found'}), 404
    body = request.get_json(force=True) or {}
    for r in habit.get('reminders', []):
        if r['id'] == rid:
            if 'time' in body:     r['time'] = body['time']
            if 'label' in body:    r['label'] = body['label']
            if 'schedule' in body: r['schedule'] = body['schedule']
            if 'enabled' in body:  r['enabled'] = body['enabled']
            save_data(data)
            return jsonify({'status': 'ok', 'reminder': r})
    return jsonify({'error': 'Reminder not found'}), 404

@app.route('/api/habit/<int:hid>/reminders/<int:rid>', methods=['DELETE'])
def delete_habit_reminder(hid, rid):
    data = load_data()
    habit = next((h for h in data['habits'] if h['id'] == hid), None)
    if not habit:
        return jsonify({'error': 'Not found'}), 404
    habit['reminders'] = [r for r in habit.get('reminders', []) if r['id'] != rid]
    save_data(data)
    return jsonify({'status': 'ok'})

# ─── Due Reminders (called by SW) ────────────────────────────────────────────

@app.route('/api/due-reminders')
def api_due_reminders():
    """Return habit reminders that are due right now (within ±3 min window)."""
    data = load_data()
    ...
    due = []
    for h in data.get('habits', []):
        for r in h.get('reminders', []):
            if not r.get('enabled', True):
                continue
            sched = r.get('schedule', 'everyday')
            if sched != 'everyday' and sched != today:
                continue
            r_time = r.get('time', '')
            if not r_time:
                continue
            # Compare times — fire if within ±3 minutes
            try:
                rt = datetime.strptime(r_time, '%H:%M')
                nt = datetime.strptime(now_hm, '%H:%M')
                diff = abs((rt.hour * 60 + rt.minute) - (nt.hour * 60 + nt.minute))
                if diff <= 3:
                    done_today = h['id'] in data.get('habit_logs', {}).get(today, [])
                    due.append({
                        'id': f"rem_{h['id']}_{r['id']}",
                        'title': f"⏰ Habit Reminder: {h['name']}",
                        'body': r.get('label') or ('Already done today! ✅' if done_today else 'Time to complete your habit'),
                        'url': f"/habit/{h['id']}",
                        'tag': f"habit_{h['id']}_reminder"
                    })
            except:
                pass
...
def get_notifications():
    ...
    return jsonify({'notifications': notifs[-50:][::-1]})

@app.route('/api/notifications', methods=['POST'])
def add_notification():
    data = load_data()
    body = request.get_json(force=True) or {}
    if 'notification_history' not in data:
        data['notification_history'] = []
    nid = max((n.get('id', 0) for n in data['notification_history']), default=0) + 1
    notif = {
        'id': nid,
        ...
        'url': body.get('url', '/'),
        'ts': datetime.now(_IST).strftime('%Y-%m-%d %H:%M'),
        'read': False
    }
    data['notification_history'].append(notif)
    # Keep only last 200
    if len(data['notification_history']) > 200:
        data['notification_history'] = data['notification_history'][-200:]
    save_data(data)
...
def mark_notifications_read():
    data = load_data()
    body = request.get_json(force=True) or {}
    nid = body.get('id')  # None = mark all
    for n in data.get('notification_history', []):
...
@app.route('/api/smart-reminders')
def smart_reminders():
    """Return smart reminders for notification system."""
    data = load_data()
    today = today_str()
    today_d = date.today()
    reminders = []
    ...
    incomplete_habits = [h for h in data.get('habits', []) if h['id'] not in done_today]
    if incomplete_habits:
        names = ', '.join(h['name'] for h in incomplete_habits[:2])
        suffix = f' +{len(incomplete_habits)-2} more' if len(incomplete_habits) > 2 else ''
        reminders.append({'type': 'habit', 'title': '📋 Habits Pending', 'body': f'{names}{suffix}', 'url': '/habits', 'priority': 'medium'})

    if total_habits > 0 and len(done_today) == total_habits:
        reminders.append({'type': 'habit_complete', 'title': '🏆 All Habits Done!', 'body': 'You completed all your habits today. Amazing!', 'url': '/habits', 'priority': 'low'})

    # Exams soon
    for ex in data.get('exams', []):
        if ex.get('date'):
            try:
                diff = (date.fromisoformat(ex['date']) - today_d).days
                if 0 <= diff <= 7:
                    reminders.append({'type': 'exam', 'title': f'📅 Exam: {ex["name"]}', 'body': 'Today!' if diff == 0 else f'In {diff} days', 'url': f'/exam/{ex["id"]}', 'priority': 'high'})
            except:
                pass

    # Weekly report (Monday)
    if today_d.weekday() == 0:
        reminders.append({'type': 'weekly_report', 'title': '📊 Weekly Report Ready', 'body': 'Your weekly productivity summary is available', 'url': '/insights', 'priority': 'low'})

    # Monthly report (1st of month)
    if today_d.day == 1:
        reminders.append({'type': 'monthly_report', 'title': '📈 Monthly Report Ready', 'body': 'Your monthly performance review is ready', 'url': '/insights', 'priority': 'low'})
...
def daily_summary():
    """Return complete daily summary for a given date."""
    data = load_data()
    d = request.args.get('date', today_str())

    # Habits
    done = set(data.get('habit_logs', {}).get(d, []))
    habits = data.get('habits', [])
    completed_habits = [h for h in habits if h['id'] in done]
    missed_habits = [h for h in habits if h['id'] not in done]
    ...
    for exam in data.get('exams', []):
        for subj in exam.get('subjects', []):
            for ch in subj.get('chapters', []):
                for sess in ch.get('study_sessions', []):
                    if sess.get('date') == d:
                        study_mins += sess.get('duration', 0) * 60
                if ch.get('completion_date') == d:
                    chapters_done.append({'name': ch['name'], 'subject': subj['name']})
    for subj in data.get('subjects', []):
        for ch in subj.get('chapters', []):
            for sess in ch.get('study_sessions', []):
                if sess.get('date') == d:
    ...
    # Exercise
    workout_logs = data.get('workout_logs', {}).get(d, [])
    exercise_mins = sum(l.get('duration_minutes', 0) for l in workout_logs)

    # Sleep
    sleep_log = next((l for l in data.get('sleep_logs', []) if l.get('date') == d), None)

    # Spending
    day_expenses = [e for e in data.get('expenses', []) if e.get('date') == d]
    day_spend = sum(e['amount'] for e in day_expenses)

    # Mood
    mood = data.get('mood_logs', {}).get(d)
    ...
    return jsonify({
        ...
        'habits': {
            ...
            'missed_names': [h['name'] for h in missed_habits],
            'completion_pct': round(len(completed_habits) / len(habits) * 100) if habits else 0
        },
        'study': {
            'minutes': round(study_mins),
            'hours': round(study_mins / 60, 1),
            'chapters_completed': chapters_done
        },
        'exercise': {
            'sessions': len(workout_logs),
            'minutes': exercise_mins
        },
        'sleep': sleep_log,
        'spending': {'total': day_spend, 'transactions': len(day_expenses)},
        'mood': mood
    })
...
def weekly_report_api():
    ...
    for h in data.get('habits', []):
        habit_completions[h['id']] = {'name': h['name'], 'done': 0}
    ...
    for i in range(6, -1, -1):
        ...
        for hid in done:
            if hid in habit_completions:
                habit_completions[hid]['done'] += 1
        ...
        e_mins = sum(l.get('duration_minutes', 0) for l in data.get('workout_logs', {}).get(d, []))
        total_study_mins += s_mins
        total_exercise_mins += e_mins
        ...
        report['days'].append({
            'date': d, 'score': score,
            'habits_done': len(done), 'total_habits': len(data.get('habits', [])),
            'study_mins': round(s_mins), 'exercise_mins': e_mins
        })

    best_habits = sorted(habit_completions.values(), key=lambda x: x['done'], reverse=True)
    report['summary'] = {
        'avg_score': round(total_score / score_count) if score_count else None,
        'total_study_hours': round(total_study_mins / 60, 1),
        'total_exercise_mins': total_exercise_mins,
        'best_habits': best_habits[:3],
        'habit_streaks': {h['id']: calc_streak(data, h['id']) for h in data.get('habits', [])}
    }
    return jsonify(report)

# ─── Habit Streak Leaderboard ─────────────────────────────────────────────────

@app.route('/api/habit-streaks')
def habit_streaks():
    data = load_data()
    streaks = []
    for h in data.get('habits', []):
        streak = calc_streak(data, h['id'])
        # 30-day rate
        rate = sum(1 for i in range(30) if h['id'] in data['habit_logs'].get((date.today() - timedelta(days=i)).isoformat(), [])) 
        streaks.append({'id': h['id'], 'name': h['name'], 'streak': streak, 'rate_30d': round(rate/30*100), 'color': h.get('color', '#4338ca')})
    streaks.sort(key=lambda x: x['streak'], reverse=True)
    return jsonify({'streaks': streaks})

# ─── Reports redirect → now a real page ──────────────────────────────────────
...
def monthly_report():
    return redirect(url_for('insights') + '?tab=monthly')
...
def test_notification():
    ...
    body = request.get_json(force=True) or {}
    notif = {
        'title': body.get('title', '🔔 AURIX Test Notification'),
        'body':  body.get('body',  'Notifications are working correctly!'),
        'url':   body.get('url',   '/habits'),
        'tag':   'aurix-test-' + datetime.now(_IST).strftime('%H%M%S'),
        'type':  'test'
    }
    # Also log it to notification history
    data = load_data()
    if 'notification_history' not in data:
        data['notification_history'] = []
    nid = max((n.get('id', 0) for n in data['notification_history']), default=0) + 1
    data['notification_history'].append({
        'id': nid, 'title': notif['title'], 'body': notif['body'],
        'type': 'test', 'url': notif['url'],
        'ts': datetime.now(_IST).strftime('%Y-%m-%d %H:%M'), 'read': False
    })
    if len(data['notification_history']) > 200:
        data['notification_history'] = data['notification_history'][-200:]
    save_data(data)
...
@app.route('/api/test-reminder-1min', methods=['POST'])
def test_reminder_1min():
    """Return a fake due reminder (for testing the trigger pipeline)."""
    body   = request.get_json(force=True) or {}
    hid    = body.get('habit_id', 0)
    hname  = body.get('habit_name', 'Test Habit')
    notif  = {
        'id':    f'test_rem_{hid}_{datetime.now(_IST).strftime("%H%M%S")}',
        'title': f'⏰ Reminder Test: {hname}',
        'body':  'This is a test — your real reminders will fire at the scheduled time.',
        'url':   f'/habit/{hid}' if hid else '/habits',
        'tag':   f'test_rem_{hid}'
    }
    return jsonify({'reminders': [notif]})

@app.route('/api/reminder-status')
def reminder_status():
    """Debug endpoint — show all enabled reminders and their next fire time."""
    data = load_data()
    now_ist = datetime.now(_IST)
    today   = now_ist.strftime('%Y-%m-%d')
    now_hm  = now_ist.strftime('%H:%M')
    ...
    for h in data.get('habits', []):
        for r in h.get('reminders', []):
            ...
            status.append({
                ...
                'label':      r.get('label', ''),
                'is_due_now': is_due,
                'fires_today': sched == 'everyday' or sched == today,
            })
    return jsonify({
        'server_time_ist': now_ist.strftime('%Y-%m-%d %H:%M:%S'),
        'today': today,
        'now_hm': now_hm,
        'total_reminders': len(status),
        'enabled_reminders': len([s for s in status if s['enabled']]),
        'due_now': len([s for s in status if s['is_due_now']]),
        'reminders': status
    })
